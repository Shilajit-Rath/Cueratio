...
SCHEMA = {
    ...
    '27_OPD_Medical_Services': ID_COLS + [
        ...
        'opd_max_limit_value', 'opd_max_limit_unit',
        'si_applicability', 'special_conditions', 'raw_text',
    ],
...
ID_FILL = PatternFill('solid', start_color='D9E1F2')
...
def build_workbook(products: list[dict], cond_data: Optional[dict]) -> Workbook:
    ...
    # --------- Populate 00_Index
    populate_index(ws_idx, products, conditions_list, registry)
...
# INDEX SHEET & STYLING
# =====================================================================
def populate_index(ws, products, conditions_list, registry):
    title_font = Font(name='Arial', size=16, bold=True, color='FFFFFF')
    title_fill = PatternFill('solid', start_color='1F4E78')
    h2_font = Font(name='Arial', size=12, bold=True, color='FFFFFF')
    ...
    c = ws['A8']
    c.value = 'Sheet Inventory'
    c.font = h2_font; c.fill = h2_fill
    ...
    inventory.extend([
        ('29_Misc_&_Extra_Benefits', 'Death, TPD, WoP, Hospital Income etc.', '~28×products', 'STRONG', 'Per-benefit rows'),
    ...
    zebra1 = PatternFill('solid', start_color='F2F2F2')
    zebra2 = PatternFill('solid', start_color='FFFFFF')
    strong = PatternFill('solid', start_color='C6EFCE')
    partial = PatternFill('solid', start_color='FFEB9C')
    na = PatternFill('solid', start_color='F8CBAD')

    for i, (s, desc, rcount, cov, n) in enumerate(inventory):
        r = 10 + i
        ws.cell(row=r, column=1, value=s).font = Font(name='Arial', size=10, bold=True)
        ws.cell(row=r, column=2, value=desc).font = body
        ws.cell(row=r, column=3, value=rcount).font = body
        ws.cell(row=r, column=4, value=cov).font = body
        ws.cell(row=r, column=5, value=n).font = body
        ...
        for cc in range(1, 6):
            ...
            if cc != 4:
                cell.fill = fill
        cov_cell = ws.cell(row=r, column=4)
        if cov in ('Full', 'STRONG', 'Good'): cov_cell.fill = strong
        elif cov in ('Partial', 'Inferred'): cov_cell.fill = partial
        elif cov == 'NA': cov_cell.fill = na
        cov_cell.alignment = Alignment(horizontal='center', vertical='center')

    last_r = 10 + len(inventory)

    # Legend section
    next_r = last_r + 2
    ws.merge_cells(start_row=next_r, start_column=1, end_row=next_r, end_column=5)
    c = ws.cell(row=next_r, column=1, value='Tab Color Legend')
    c.font = h2_font; c.fill = h2_fill
    c.alignment = Alignment(horizontal='left', vertical='center', indent=1)
    ws.row_dimensions[next_r].height = 22

    legend = [
        ('Green', 'Cleanly populated with source data', '00B050'),
        ('Yellow', 'Schema populated but limited source data', 'FFC000'),
        ('Grey', 'Utility / validation lists', '808080'),
    ]
    for i, (color, desc, hexcode) in enumerate(legend):
        r = next_r + 1 + i
        cc = ws.cell(row=r, column=1, value=color)
        cc.fill = PatternFill('solid', start_color=hexcode)
...
def set_tab_colors(wb):
    ...
    UTIL = '808080'
    good = {
        '00_Index', '01_Company_Master', '02_Product_Master', '03_Variant_Master',
        '04_Entry_Age', '05_Policy_Term', '06_Coverage', '11_Payment_Frequency',
        '08_Policy_Year_Benefit_Amt', '28_Critical_Illness_Benefits',
        '28A_CI_Conditions_Coverage', '29_Misc_&_Extra_Benefits',
    }
    for sn in wb.sheetnames:
        if sn in good:
            wb[sn].sheet_properties.tabColor = GOOD
        elif sn in ('00_Validations', 'Iterations_Status'):
            wb[sn].sheet_properties.tabColor = UTIL
        else:
            wb[sn].sheet_properties.tabColor = PARTIAL
